...
public func weakify <T: AnyObject, U, D>(_ owner: T, _ f: @escaping (T) -> (U, D) -> Void) -> (U, D) -> Void {
...
public extension Weakable {
	func weak<T: AnyObject>(_ f: @escaping (T) -> () -> Void) -> () -> Void {
		return weakify(self as! T, f)
	}

	func weak<T: AnyObject, U>(_ f: @escaping (T) -> (U) -> Void) -> (U) -> Void {
		return weakify(self as! T, f)
	}

	func weak<T: AnyObject, U, D>(_ f: @escaping (T) -> (U, D) -> Void) -> (U, D) -> Void {
		return weakify(self as! T, f)
	}

	func weak<U>(_ f: @escaping (Self) -> (U) -> Void) -> (U) -> Void {
		return weakify(self, f)
	}
}
